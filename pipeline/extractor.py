...
class ClaimExtractor:
    ...
    def _to_int(val) -> Optional[int]:
        ...
        try:
            ...
            return None    TEAM_SIZE     = "team_size"
    OTHER         = "other"
...
class LLMAdapter:
    """
    Adapter generico per chiamate LLM.
    Supporta: Groq, Ollama, OpenAI-compatible endpoints.

    Uso con Groq (free tier):
        adapter = LLMAdapter(
            provider="groq",
            api_key="gsk_...",
    ...
    Uso con Ollama (locale, costo zero):
        adapter = LLMAdapter(
            provider="ollama",
            model="llama3.1:8b",
    ...
    @classmethod
    def for_gemini(cls, api_key: str, model: str = "gemini-2.0-flash") -> "LLMAdapter":
        """Shortcut per creare un adapter Gemini (Google AI Studio)."""
        return cls(provider="gemini", api_key=api_key, model=model)
